...
class ElGalloSpinService3 extends AbstractSpinService
{
    ...
    {
        $result = $this->getSpinData($betLine, $linesInGame, $gameName, 3);

        return $result;
    }

    //$this->getGameRules($gameName);
    public function getSpinData($betLine, $linesInGame, $gameName, $recoil)
    {
        $this->recoil = $recoil;
        ...
        //$info = [8,7,9,6,7,8,6,7,8,4,2,8,2,11,1];
    ...
    public function getJackpot()
    {
        ...
        if ($jackpots['MINI'] >= $data['result_mini']) {
            session(['allWin' => (session('allWin') + $data['mini'])]);
            $this->allWin = session('allWin');
            $jackpots['result'] = 'MINI';

            // замена значения для джекпота, для того, чтобы не выводилось большее чем значение, чем выигрышное
            $jackpots['MINI'] = $data['result_mini'];
        ...
        if ($jackpots['MINOR'] >= $data['result_minor']) {
            session(['allWin' => (session('allWin') + $data['minor'])]);
            $this->allWin = session('allWin');
        ...
        if ($jackpots['MAJOR'] >= $data['result_major']) {
            session(['allWin' => (session('allWin') + $data['major'])]);
            $this->allWin = session('allWin');
        ...
        if ($jackpots['BIG DADDY'] >= $data['result_big_daddy']) {
            session(['allWin' => (session('allWin') + $data['big_daddy'])]);
            $this->allWin = session('allWin');
    ...
    private function getRope($repeat = 'start')
    {
        ...
        if ($repeat === false) {
            ...
            $freeSpinData = ['count' => $countsArray[array_rand($countsArray)], 'mul' => rand(1, 5), 'allWin' => 0, 'repeat' => false, 'hash' => rand(0, 10000)];
        } else {
            return 'repeat error';
        }
