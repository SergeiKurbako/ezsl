...
class ElGalloSpinService2 extends AbstractSpinService
{
    ...
    public function getJackpot()
    {
        ...
        $jackpotKey = array_rand($jackpots);

        if (!rand(0,0)) {
            //if (1) {
            session(['allWin' => (session('allWin') + floor($jackpots[$jackpotKey]))]);
            $this->allWin = session('allWin');
            $jackpots['result'] = $jackpotKey;

            // обнуляем джекпот выигравшей категории
            $this->resetJackpot($jackpotKey);
    ...
    public function resetJackpot(string $jackpotKey)
    {
        ...
        switch ($jackpotKey) {
            case 'MINI' :
                ...
                $jackpotDB->mini = ($jackpotDB->min_mini + $remainder);
                break;
            case 'MINOR' :
                $remainder = $jackpotDB->minor - floor($jackpotDB->minor);
                $jackpotDB->minor = ($jackpotDB->min_minor + $remainder);
                break;
            case 'MAJOR' :
                $remainder = $jackpotDB->major - floor($jackpotDB->major);
                $jackpotDB->major = ($jackpotDB->min_major + $remainder);
                break;
            case 'BIG DADDY' :
                $remainder = $jackpotDB->big_daddy - floor($jackpotDB->big_daddy);
                $jackpotDB->big_daddy = ($jackpotDB->min_big_daddy + $remainder);
                break;
        }
